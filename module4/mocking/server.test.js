    ...
    describe("PUT /api/products/:id", () => {
        ...
        it("updateProduct_whenValidDataProvided_returns200WithUpdatedProduct", async () => {
            ...
            expect(db.query).toHaveBeenCalledWith(
              ...
              ["Laptop", 999.99, PRODUCT_ID]
            );
        });

        it("updateProduct_whenProductDoesNotExist_returns404WithErrorMessage", async () => {
            // RETURNING * yields no rows when the WHERE clause matches nothing
            db.query.mockResolvedValueOnce({ rows: [] });
        ...
        it("updateProduct_whenDatabaseFails_returns500WithErrorMessage", async () => {
            db.query.mockRejectedValueOnce(new Error("DB error"));
